...
@router.post("/gerar")
def gerar_link(body: dict, request: Request, db: Session = Depends(SessionLocal)):
    tabela_id = int(body["tabela_id"])
    com_frete = bool(body["com_frete"])
    # pega data_prevista (pode vir ausente/nula)
    data_prevista_str = body.get("data_prevista")

    # chama o service já passando a data
    code, expires_at, data_prevista = gerar_link_code(db, tabela_id, com_frete, data_prevista_str)
...
@router.get("/resolver")
def resolver_link(code: str, db: Session = Depends(SessionLocal)):
    link, status = resolver_code(db, code)  # usa a função importada de services
    if status == "not_found":
