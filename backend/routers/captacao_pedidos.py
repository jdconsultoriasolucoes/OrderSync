    ...
    try:
        ...
        for c in clientes:
            ...
            if ultima_compra_data:
                # Se for string, tenta converter (caso venha do SQLite ou similar sem datetime nativo)
                if isinstance(ultima_compra_data, str):
                    ultima_compra_data = parse_date(ultima_compra_data)
                
                if ultima_compra_data:
                    # Garantir que trabalhamos apenas com a data, não timestamp
            ...
            # Se não tem previsão, joga pro fim da lista (infinito positivo)
            # Usa "date" em vez de "datetime" para não dar TypeError ao comparar com previsao_data que é "date"
            import datetime as dt_mod
