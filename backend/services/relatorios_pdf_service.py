...
def gerar_pdf_formacao_carga(db, carga_id: int) -> bytes:
    ...
    # 3. Draw PDF
    total_liq = sum(getattr(p, 'peso_liquido_total', 0.0) or getattr(p, 'peso_total_kg', 0.0) or 0.0 for p in pedidos)
    total_bruto = sum(getattr(p, 'peso_bruto_total', 0.0) or getattr(p, 'peso_total_kg', 0.0) or 0.0 for p in pedidos)
...
def _desenhar_resumo_logic(c, db, carga, produtos, width, height, y_start=None):
    ...
    # Table columns: CÓDIGO | DESCRIÇÃO | Peso EMB. | Emb. | OBS. | QTD | P. LÍQ
    data = [["CÓDIGO", "DESCRIÇÃO", "Peso\nEMB.", "Emb.", "OBS.", "QTD", "P. LÍQ"]]
    ...
    col_widths = [1.8*cm, 7.5*cm, 1.8*cm, 1.5*cm, 3.8*cm, 1.5*cm, 1.7*cm]
    table = Table(data, colWidths=col_widths, repeatRows=1)
    style = TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), SUPRA_BAR),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('ALIGN', (2, 0), (2, -1), 'CENTER'), # Peso EMB.
        ('ALIGN', (5, 0), (6, -1), 'CENTER'), # Qtd, P. LÍQ
...
def gerar_pdf_romaneio_retirada(db, retirada_id: int) -> bytes:
    ...
    total_liq_val = sum(p.peso_liquido_total or 0 for p in pedidos)
