...
# We update all "ingestão manual" orders to FATURADO_SUPRA, except the ones in the list
cur.execute("""
    UPDATE tb_pedidos 
    SET status = 'FATURADO_SUPRA' 
    WHERE tabela_preco_nome = 'ingestão manual' 
      AND status != 'FATURADO_SUPRA'
      AND pedido_supra != ALL(%s);
""", (excluded_ids,))

updated_count = cur.rowcount
conn.commit()

print(f"Updated {updated_count} orders to 'FATURADO_SUPRA'")
